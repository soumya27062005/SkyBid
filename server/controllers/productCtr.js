...
const createProduct = asyncHandler(async (req, res) => {
  const { title, description, price, category, height, length, width, mediumused, weigth } = req.body;
  ...
  res.status(201).json({
    data: product,
    meta: {status: 1, message: 'Product added successfully.'}
  });
});

const getAllProducts = asyncHandler(async (req, res) => {
  const products = await Product.find({ isVerified: true}).sort("-createdAt").populate("user");
  ...
  res.status(200).json({data: productsWithDetails});
});
...
const getAllProductsofUser = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({data: productsWithPrices, meta: {status: 1, message: 'Products listed successfully.'}});
});
